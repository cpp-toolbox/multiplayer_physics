...
void Physics::initialize_world_objects() {
  ...
  JPH::ConvexHullShapeSettings low_poly_ball_settings(
      vertices, JPH::cDefaultConvexRadius);
  JPH::ShapeSettings::ShapeResult ball_shape_result =
      low_poly_ball_settings.Create();
  ...
  JPH::BodyCreationSettings ball_creation_settings(
      ball_shape, JPH::RVec3(5.0, 20.0, 5.0), JPH::Quat::sIdentity(),
      JPH::EMotionType::Dynamic, Layers::MOVING);
  JPH::Body *ball = body_interface.CreateBody(
      ball_creation_settings); // Note that if we run out of bodies this can
                               // return nullptr
...
void Physics::load_model_into_physics_world(Model *model) {
  ...
  for (int i = 0; i < model->meshes.size(); i++) {
    ...
    JPH::BodyCreationSettings mesh_settings(
        mesh_shape, JPH::RVec3(0.0, 0.0, 0.0), JPH::Quat::sIdentity(),
        JPH::EMotionType::Static, Layers::NON_MOVING);
    JPH::Body *mesh_body = body_interface.CreateBody(
        mesh_settings); // Note that if we run out of bodies this can return
                        // nullptr
...
void Physics::create_character(uint64_t client_id) {
  ...
  settings->mShape = new JPH::CapsuleShape(0.5f * this->character_height,
                                           this->character_radius);
  ...
  JPH::Ref<JPH::CharacterVirtual> character =
      new JPH::CharacterVirtual(settings, JPH::RVec3(0.0f, 10.0f, 0.0f),
                                JPH::Quat::sIdentity(), &physics_system);

  client_id_to_physics_character[client_id] = character;
}

void Physics::delete_character(uint64_t client_id) {
...
void Physics::update(float delta_time) {
  ...
  // std::cout << "Character UP = (" << ch_up.GetX() << ", " << ch_up.GetY() <<
  // ", " << ch_up.GetZ() << ")" << std::endl;
  ...
  for (const auto &pair : client_id_to_physics_character) {
    ...
    character->ExtendedUpdate(
        delta_time, -character->GetUp() * physics_system.GetGravity().Length(),
        update_settings,
  ...
  physics_system.Update(delta_time, cCollisionSteps, temp_allocator,
                        job_system);
}
